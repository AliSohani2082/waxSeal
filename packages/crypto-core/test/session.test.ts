...
describe("encryptMessage / decryptMessage", () => {
	...
	it("throws DecryptError when decrypting with the wrong key", async () => {
		...
		await expect(decryptMessage(keyB, iv, ciphertext)).rejects.toThrow(
			DecryptError,
		);
	});
	...
	it("throws DecryptError when the ciphertext is tampered with", async () => {
		...
		await expect(decryptMessage(key, iv, tampered)).rejects.toThrow(
			DecryptError,
		);
	});
	...
	it("throws DecryptError when the IV is wrong", async () => {
		...
		await expect(decryptMessage(key, wrongIv, ciphertext)).rejects.toThrow(
			DecryptError,
		);
	});
});
