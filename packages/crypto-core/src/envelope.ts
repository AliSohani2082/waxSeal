import { base64UrlEncode, base64UrlDecode, Base64UrlDecodeError, concatBytes } from "./bytes.js";
...
function assertLen(name: string, bytes: Uint8Array, expected: number): void {
  if (bytes.length !== expected) {
    throw new EnvelopeDecodeError(`${name} must be ${expected} bytes, got ${bytes.length}`);
  }
}
...
export function decodeEnvelope(token: string): EnvelopeFields {
  if (!token.startsWith(MARKER)) {
    throw new EnvelopeDecodeError("token does not start with the waxseal marker");
  }
  ...
  try {
    ...
    if (err instanceof Base64UrlDecodeError) {
      throw new EnvelopeDecodeError(`invalid base64url payload: ${err.message}`);
    }
    throw err;
  }
