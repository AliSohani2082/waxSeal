...
description: Source of truth for Waxseal's cryptographic design. Code in packages/crypto-core must match this document.
---
...
- One identity per device (not per account, not per site). Multi-device sync
  is a Phase 3 feature — see [Multi-Device Sync](/docs/sync/design).
...
- Fingerprint = human-readable rendering of `SHA-256(canonical public key JWK bytes)`.
...
**Future upgrade path**: migrate session establishment to ECDH (P-256 via
...
- No multi-device support: one identity keypair per device, so a contact's
  ...
  key-change warning, as designed). See [Multi-Device Sync](/docs/sync/design) for the Phase 3 design.
