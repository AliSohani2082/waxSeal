---
title: Multi-Device Sync Design
description: Source of truth for Waxseal's Phase 3 multi-device key synchronisation. Extends the Cryptography Design.
---
...
Read [Cryptography Design](/docs/crypto/design) first — this document extends it.
...
can safely derive multiple independent keys in the future.
...
export async function deriveBundleKey(
  ...
  bundleSalt: Uint8Array  // 32-byte per-user salt
...
duration of the sync operation and then zeroed.

## Key storage upgrade (ExtractableKeyStore)

Phase 3 adds `ExtractableKeyStore` behind the existing `KeyStore` interface
in `extension-core/src/storage.ts`. Keys are stored as JWK in IndexedDB.
...
1. Warn the user: "Multi-device sync requires regenerating your identity key. Your contacts will need to re-verify your safety number."
...
This is a one-time migration, documented plainly in the UI.
...
- **No post-quantum security**: OPAQUE-P256 and ECDH-P256 are vulnerable to
  a sufficiently large quantum computer. Migration is feasible because the
...
- [@serenity-kit/opaque-p256](https://www.npmjs.com/package/@serenity-kit/opaque-p256)
