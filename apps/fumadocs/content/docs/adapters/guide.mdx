---
title: Writing a Site Adapter
description: How to implement a SiteAdapter to add Waxseal support for a new chat platform.
---
...
Copy the template with `pnpm tsx scripts/new-adapter.ts <site-id>`, or copy
`packages/adapters/adapter-generic` by hand as a starting point.

### 2. Inspect the target site

Open the target site and use devtools to find:
...
### 3. Write tests against static HTML fixtures

Write unit tests against **static HTML fixtures** (a saved snapshot of the
...
### 4. Register the adapter and update the manifest

Register the adapter and add its required host permission to both
...
Because sites change their DOM without notice, expect the fixture and
